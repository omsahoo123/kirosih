...
export default function WomenSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "women", name: "Demo Women User", email: "women@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser("women@aarogya.ai", password);
  ...
  return (
    <div>
      ...
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <p className="text-gray-500 font-mono text-xs">women@aarogya.ai / Test@1234</p>
        <button type="button" onClick={() => { setIdentifier("8765432109"); setMode("password"); setPassword("Test@1234"); }}
