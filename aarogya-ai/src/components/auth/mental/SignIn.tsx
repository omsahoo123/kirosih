...
export default function MentalSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const sendOtp = async () => {
    if (!identifier) { setError("Enter your mobile number or Anonymous ID"); return; }
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "mental", name: "Demo Mental User", email: "mental@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser("mental@aarogya.ai", password);
  ...
  return (
    <div>
      ...
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <p className="text-gray-500 font-mono text-xs">mental@aarogya.ai / Test@1234</p>
        <button type="button" onClick={() => { setIdentifier("8098765432"); setMode("password"); setPassword("Test@1234"); }}
      ...
      <div className="mb-5">
        <FieldLabel required>Mobile Number / Anonymous ID</FieldLabel>
        <InputBase type="text" value={identifier} onChange={e => { setIdentifier(e.target.value); setError(""); }}
          placeholder="9876543210 or anon_7f3k" error={!!error && !identifier} />
      </div>
