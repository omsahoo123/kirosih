...
export default function HospitalSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const sendOtp = async () => {
    if (!identifier) { setError("Enter your email, mobile, or Hospital ID"); return; }
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "hospital", name: "Demo Hospital Admin", email: "hospital@aarogya.ai" }));
    router.push("/dashboard/hospital");
  };

  const loginWithPassword = async () => {
    if (!identifier || !password) { setError("Enter your identifier and password"); return; }
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser(identifier.includes("@") ? identifier : "hospital@aarogya.ai", password);
  ...
  return (
    <div>
      <SectionHeading title="Hospital Portal" subtitle="Sign in to your AarogyaAI hospital admin account." />
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <p className="text-gray-500 font-mono text-xs">hospital@aarogya.ai / Test@1234</p>
        <button type="button" onClick={() => { setIdentifier("hospital@aarogya.ai"); setMode("password"); setPassword("Test@1234"); }}
      ...
      <div className="mb-5">
        <FieldLabel required>Email / Mobile / Hospital ID</FieldLabel>
        <InputBase type="text" value={identifier} onChange={e => { setIdentifier(e.target.value); setError(""); }}
          placeholder="admin@hospital.com or MH-HOS-2005" error={!!error && !identifier} />
      </div>
            ...
            <>
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-gray-400">Sent to {identifier}</span>
                </div>
