...
export default function DoctorSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const sendOtp = async () => {
    if (!identifier) { setError("Enter your mobile, email, or registration number"); return; }
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "doctor", name: "Demo Doctor", email: "doctor@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser(identifier.includes("@") ? identifier : "doctor@aarogya.ai", password);
  ...
  return (
    <div>
      <SectionHeading title="Welcome back, Doctor" subtitle="Sign in to your AarogyaAI clinician account." />
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <p className="text-gray-500 font-mono text-xs">doctor@aarogya.ai / Test@1234</p>
        <button type="button" onClick={() => { setIdentifier("doctor@aarogya.ai"); setMode("password"); setPassword("Test@1234"); }}
      ...
      <div className="mb-5">
        <FieldLabel required>Mobile / Email / Registration No.</FieldLabel>
        <InputBase type="text" value={identifier} onChange={e => { setIdentifier(e.target.value); setError(""); }}
          placeholder="9876543210, doctor@email.com or MCI-XXXX" error={!!error && !identifier} />
      </div>
