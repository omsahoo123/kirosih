...
import { User, Calendar, ChevronRight, ChevronLeft, MapPin } from "lucide-react";
...
const INIT: FormData = {
  full_name: "", dob: "", gender: "", blood_group: "", medical_history: "", preferred_language: "",
  state: "", district: "",
  phone: "", phone_otp: "",
...
export default function RuralSignUp({ onSwitchToSignIn }: { onSwitchToSignIn: () => void }) {
  ...
  return (
    <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={form.medical_history} onChange={e => set("medical_history", e.target.value)}
                placeholder="e.g. Diabetes, Hypertension... (optional)" rows={3}
            ...
            <div>
              ...
              <SelectBase value={form.preferred_language} onChange={e => set("preferred_language", e.target.value)}>
                <option value="">Select language</option>
                {INDIAN_LANGUAGES.map(l => <option key={l}>{l}</option>)}
              </SelectBase>
              <p className="text-[10px] text-gray-400 mt-1.5">AarogyaAI will show content in your preferred language.</p>
            </div>
          </div>
        </>
      )}
      ...
      {step === 2 && (
        <>
          <SectionHeading step={2} total={TOTAL_STEPS} title="Location & Mobile" subtitle="Your location helps us find nearby PHC and route 108 emergencies." />
          <div className="space-y-4">
            <div className="rounded-2xl border border-gray-100 bg-gray-50/50 p-4 space-y-3">
              <div className="flex items-center gap-2 mb-1">
                <MapPin className="w-4 h-4 text-[#1A6B3C]" />
                <span className="text-sm font-bold text-[#0F4024]">Your Location</span>
              </div>
              <div>
              ...
              <div>
                ...
                <InputBase icon={MapPin} value={form.district} onChange={e => set("district", e.target.value)}
                  placeholder="e.g. Barmer, Datia..." error={!!errors.district} />
