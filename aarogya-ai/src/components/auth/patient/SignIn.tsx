...
export default function PatientSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const sendOtp = async () => {
    if (!identifier) { setError("Enter your phone number, Aadhaar, or ABHA ID"); return; }
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "patient", name: "Demo Patient", email: "patient@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser(identifier.includes("@") ? identifier : "patient@aarogya.ai", password);
  ...
  return (
    <div>
      ...
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <button type="button" onClick={() => { setIdentifier("patient@aarogya.ai"); setMode("password"); setPassword("Test@1234"); }}
      ...
      <div className="mb-5">
        ...
        <InputBase type="text" value={identifier} onChange={e => { setIdentifier(e.target.value); setError(""); }}
          placeholder="9876543210 or yourname@abdm" error={!!error && !identifier} />
      </div>
