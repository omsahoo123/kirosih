...
export default function RuralSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "rural", name: "Demo Rural User", email: "rural@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setLoading(true);
    await new Promise((r) => setTimeout(r, 800));
    const user = findDummyUser("rural@aarogya.ai", password);
    if (!user) { setError("Incorrect mobile number or password."); setLoading(false); return; }
    sessionStorage.setItem("aarogya_user", JSON.stringify(user));
    router.push("/dashboard/rural");
  };

  const switchMode = (m: AuthMode) => { setMode(m); setError(""); setOtp(""); setOtpStage("idle"); };
  ...
  return (
    <div>
      ...
      {/* Demo hint */}
      <div className="mb-5 px-4 py-3 rounded-xl bg-[#8BC34A]/8 border border-[#8BC34A]/25 text-sm">
        <p className="font-semibold text-[#2D6A0A] mb-0.5">🧪 Demo login</p>
        <p className="text-gray-500 font-mono text-xs">rural@aarogya.ai / Test@1234</p>
      </div>
      ...
      {mode === "otp" && (
        <div className="space-y-4">
          {otpStage === "idle" && (
            <PrimaryButton onClick={sendOtp} loading={loading}>🔐 Send OTP</PrimaryButton>
          )}
          {otpStage === "sent" && (
            <>
              <div>
              ...
              <PrimaryButton onClick={verifyOtp} loading={loading}>Verify & Sign In</PrimaryButton>
      ...
      {mode === "password" && (
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <a href="#" className="text-xs text-[#1A6B3C] font-semibold hover:underline">Forgot password?</a>
          </div>
          <PrimaryButton onClick={loginWithPassword} loading={loading}>🔑 Sign In with Password</PrimaryButton>
          <p className="text-xs text-center text-gray-400">
            Prefer OTP?{" "}
