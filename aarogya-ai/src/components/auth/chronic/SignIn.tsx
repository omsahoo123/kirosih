...
export default function ChronicSignIn({ onSwitchToSignUp }: { onSwitchToSignUp: () => void }) {
  ...
  const verifyOtp = async () => {
    ...
    sessionStorage.setItem("aarogya_user", JSON.stringify({ role: "chronic", name: "Demo Chronic User", email: "chronic@aarogya.ai" }));
  ...
  const loginWithPassword = async () => {
    ...
    setError(""); setLoading(true);
    await new Promise(r => setTimeout(r, 800));
    const user = findDummyUser("chronic@aarogya.ai", password);
  ...
  return (
    <div>
      ...
      <div className="mb-5 px-4 py-3 rounded-xl border text-sm" style={{ borderColor: ACCENT + "40", backgroundColor: ACCENT + "0D" }}>
        ...
        <p className="text-gray-500 font-mono text-xs">chronic@aarogya.ai / Test@1234</p>
        <button type="button" onClick={() => { setIdentifier("9123456789"); setMode("password"); setPassword("Test@1234"); }}
